...
interface InputNumberProps extends InputHTMLAttributes<HTMLInputElement> {
  ...
  valid?: any;
  errorMessage?: any;
  styleClassico?: boolean;
  styleInput?:string;
}

export const InputNumber: React.FC<InputNumberProps> = (props) => {
  return (
    <Container color={props.color} className={props.className} typeStyle={props.styleClassico}>
      {props.styleClassico ? (
        <NumericFormat
          className={"px-1 "+props.styleInput}
          id={props.label}
          type={"text"}
        ...
        <>
          <label className="input_line__label" htmlFor={props.label} style={{ color: props.color }}>
            {props.label}
          </label>
          <NumericFormat
            className={"input_line__field "+props.styleInput}
            id={props.label}
            type={"text"}
            thousandSeparator={props.separadorMilhar}
            decimalSeparator={props.separadorDecimal}
